...
def evaluate_rm(args: Args, accelerator, tokenizer, model, ref_model, dataloader):
    ...
    with torch.no_grad():
        ...
        for data in tqdm(dataloader):
            ...
            reward_preferred = args.beta * (chosen_logps - 0 * ref_chosen_logps).detach()
            reward_rejected = args.beta * (rejected_logps - 0 * ref_rejected_logps).detach()
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for data in tqdm(dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                logits = pi_logratios - 0 * ref_logratios  # also known as h_{\pi_\theta}^{y_w,y_l}
                if args.ipo:
