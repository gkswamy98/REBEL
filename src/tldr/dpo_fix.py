...
class LabelHParams:
    ...
    num_train: int = 92832 #92832 #209580
    num_labels: int = 2
...
if __name__ == "__main__":
    ...
    dataset = dataset.with_format(
        ...
        columns=[
            ...
            # "query_token_len"
            f"iter_{args.iter - 1}_worst_query_response",
            f"iter_{args.iter - 1}_worst_mask",
            f"iter_{args.iter - 1}_best_query_response",
            f"iter_{args.iter - 1}_best_mask",
        ],
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for data in tqdm(dataloader):
            ...
            query_responses = torch.cat(
                [data[f"iter_{args.iter - 1}_best_query_response"].unsqueeze(1), data[f"iter_{args.iter - 1}_worst_query_response"].unsqueeze(1)], dim=1
            ).flatten(0, 1)
            ...
            labels =  torch.cat(
                [data[f"iter_{args.iter - 1}_best_mask"].unsqueeze(1), data[f"iter_{args.iter - 1}_worst_mask"].unsqueeze(1)], dim=1
            ).flatten(0, 1)
            ...
            # rewards_1 = get_reward(reward_model, data[f"iter_{args.iter - 1}_query_response_0"], tokenizer, 512)
            ...
            # mb_best = (rewards_0 < rewards_1).long()
            # neither_eos = torch.maximum(rewards_0, rewards_1) <= -1e5

            mb_best = torch.zeros(len(data)).long().to(device)
