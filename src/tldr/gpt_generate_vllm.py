...
class EvalStorage:
    ...
    postprocessed_responses: List[List[str]] = field(default_factory=list)
    reference_response: List[str] = field(default_factory=list)


    # prompts = [row['query'] for row in tqdm(dataset)]
    # sampling_params = [SamplingParams(temperature=0.01, top_p=1.0, max_tokens=53, seed=((args.p + 1) * 50) + i) for i in range(len(prompts))]
...
def evaluate_policy(args: Args, model, tokenizer, dataloader, dataset, sampling=True):
    ...
    generated_texts = [[output.outputs[0].text] for output in tqdm(outputs)]

    N = 2 # TODO: change to 25
    for n in tqdm(range(N - 1)):
        sampling_params = [SamplingParams(temperature=0.1, top_p=1.0, max_tokens=53, seed=((n + 1) * 50) + i) for i in range(len(prompts))]
        outputs = model.generate(prompts, sampling_params)
        for i, output in enumerate(outputs):
            generated_texts[i].append(output.outputs[0].text)

    eval_storage.postprocessed_responses = generated_texts
...
if __name__ == "__main__":
    ...
    sft_validation_dataset = load_dataset(args.label_dataset, split="train") # TODO: change to query_dataset
    sft_validation_dataset = sft_validation_dataset.with_format("torch", columns=["query"]) # TODO: add in reference_response col
