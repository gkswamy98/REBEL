...
class Args:
    # common args
    exp_name: str = "pythia_1.4_iter_dpo"
    ...
    output_dir: str = "models/iter_1_dpo_policy_model_1_4b"
...
def forward(model, query_responses, tokenizer):
    ...
    logits = outputs.logits[:, 512 - 1 : -1]
    logits /= 0.7 + 1e-7
    all_logprob = F.log_softmax(logits, dim=-1)

    response = query_responses[:, 512:]
    # seq_len = first_true_indices(response == tokenizer.pad_token_id) - 1
    # seq_mask = torch.arange(53, device=query_responses.device).unsqueeze(0).expand_as(response) <= seq_len.unsqueeze(1)

    seq_mask = response != tokenizer.pad_token_id

    logps = torch.gather(all_logprob, 2, response.unsqueeze(-1)).squeeze(-1)
    logps = logps * seq_mask
    return logps
...
if __name__ == "__main__":
    ...
    # TODO (gswamy): if regenerating the ref logprobs doesn't fix things, try running code on off-policy data
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for data in tqdm(dataloader):
            ...
            # query_responses = torch.cat(
            #     [data[f"iter_{args.iter - 1}_query_response_0"].unsqueeze(1),
            #      data[f"iter_{args.iter - 1}_query_response_1"].unsqueeze(1)], dim=1
            # ).flatten(0, 1)
            # rewards = torch.cat(
            #     [data[f"iter_{args.iter - 1}_reward_0"].unsqueeze(1),
            #      data[f"iter_{args.iter - 1}_reward_1"].unsqueeze(1)],
            #     dim=1,
            # ).flatten(0, 1)
            # reg_log_probs = torch.cat(
            #     [data[f"iter_{args.iter - 1}_logprob_0"].unsqueeze(1),
            #      data[f"iter_{args.iter - 1}_logprob_1"].unsqueeze(1)],
            #     dim=1, 
            # ).flatten(0, 1)
            # mb_best = labels[:, 0] > labels[:, 1]  
            # with torch.no_grad():
                # ref_chosen_logps, ref_rejected_logps = forward(ref_model, query_responses, labels, mb_best, tokenizer)
            # rm_score_0 = data[f"iter_{args.iter - 1}_reward_0"]
            # rm_score_1 = data[f"iter_{args.iter - 1}_reward_1"]

            # ref_logps_seq_0 = data[f"iter_{args.iter - 1}_logprob_0"]
            # ref_logps_seq_1 = data[f"iter_{args.iter - 1}_logprob_1"]

            rewards_0 = get_reward(reward_model, data[f"iter_{args.iter - 1}_query_response_0"], tokenizer, 512)
            rewards_1 = get_reward(reward_model, data[f"iter_{args.iter - 1}_query_response_1"], tokenizer, 512)

            response_0 = data[f"iter_{args.iter - 1}_query_response_0"][:, 512:]
            # response_0 = data["query_response0_token"][:, 512:]
            # print(response_0.shape)
            # seq_len_0 = first_true_indices(response_0 == tokenizer.pad_token_id) - 1
            # print(seq_len_0.shape)
            # seq_mask_0 = torch.arange(53, device=response_0.device).unsqueeze(0).expand_as(response_0) <= seq_len_0.unsqueeze(1)
            seq_mask_0 = response_0 != tokenizer.pad_token_id

            response_1 = data[f"iter_{args.iter - 1}_query_response_1"][:, 512:]
            # response_1 = data["query_response1_token"][:, 512:]
            # seq_len_1 = first_true_indices(response_1 == tokenizer.pad_token_id) - 1
            # seq_mask_1 = torch.arange(53, device=response_1.device).unsqueeze(0).expand_as(response_1) <= seq_len_1.unsqueeze(1)
            seq_mask_1 = response_1 != tokenizer.pad_token_id

            # ref_logps_seq_0 = ref_logps_seq_0 * seq_mask_0
            # ref_logps_seq_1 = ref_logps_seq_1 * seq_mask_1
            ...
            # zero_wins = data[f"iter_{args.iter - 1}_reward_0"] > data[f"iter_{args.iter - 1}_reward_1"]
            # ref_logps_0 = data[f"iter_{args.iter - 1}_logprob_0"].sum(1)
            # ref_logps_1 = data[f"iter_{args.iter - 1}_logprob_1"].sum(1)
            # ref_chosen_logps = torch.where(zero_wins, ref_logps_0, ref_logps_1)
            # ref_rejected_logps = torch.where(zero_wins, ref_logps_1, ref_logps_0)

            with accelerator.accumulate(model):
                logps_seq_0 = forward(model, data[f"iter_{args.iter - 1}_query_response_0"], tokenizer)
                logps_seq_1 = forward(model, data[f"iter_{args.iter - 1}_query_response_1"], tokenizer)
                # logps_seq_0 = forward(model, data["query_response0_token"], tokenizer)
                # logps_seq_1 = forward(model, data["query_response1_token"], tokenizer)

                logps_seq_0 = logps_seq_0 * seq_mask_0
                logps_seq_1 = logps_seq_1 * seq_mask_1

                ref_logps_seq_0 = forward(ref_model, data[f"iter_{args.iter - 1}_query_response_0"], tokenizer)
                ref_logps_seq_1 = forward(ref_model, data[f"iter_{args.iter - 1}_query_response_1"], tokenizer)

                # ref_logps_seq_0 = forward(ref_model, data["query_response0_token"], tokenizer)
                # ref_logps_seq_1 = forward(ref_model, data["query_response1_token"], tokenizer)

                ref_logps_seq_0 = ref_logps_seq_0 * seq_mask_0
                ref_logps_seq_1 = ref_logps_seq_1 * seq_mask_1

                # kl_0 = logps_seq_0.sum(-1) - ref_logps_seq_0.sum(-1)
                # kl_1 = logps_seq_1.sum(-1) - ref_logps_seq_1.sum(-1)

                # first_wins = rm_score_0 - 0.05 * kl_0 > rm_score_1 - 0.05 * kl_1
                # first_wins = rm_score_0  > rm_score_1
                first_wins = (rewards_0[1] < rewards_1[1])

                ref_chosen_logps = torch.where(first_wins, ref_logps_seq_0.sum(-1), ref_logps_seq_1.sum(-1))
                ref_rejected_logps = torch.where(first_wins, ref_logps_seq_1.sum(-1), ref_logps_seq_0.sum(-1))
                chosen_logps = torch.where(first_wins, logps_seq_0.sum(-1), logps_seq_1.sum(-1))
                rejected_logps = torch.where(first_wins, logps_seq_1.sum(-1), logps_seq_0.sum(-1))
