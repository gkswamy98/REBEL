...
def main():
    ...
    for p in range(args.p, args.p + 1):
        ...
        for data in tqdm(dataloader):
            ...
            del contain_pad_token, scores, postprocessed_responses, postprocessed_query_responses, rewards, masks
            torch.cuda.empty_cache()
        ...
        torch.save(all_query_responses, f"./temp_datastore/all_query_responses_iter_{args.iter}_samp_{p}_ws.pt")
        torch.save(all_masks, f"./temp_datastore/all_masks_iter_{args.iter}_samp_{p}_ws.pt")
        torch.save(all_decoded, f"./temp_datastore/all_decoded_iter_{args.iter}_samp_{p}_ws.pt")
        torch.save(all_rewards, f"./temp_datastore/all_rewards_iter_{args.iter}_samp_{p}_ws.pt")
