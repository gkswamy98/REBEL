...
class Args:
    ...
    gradient_accumulation_steps: int = 16 # 32 / (num_gpus) = gradient_accumulation_steps
    ...
    label_dataset: str = "gswamy/pythia-1.4b-tldr-spin-iter-"
...
def evaluate_rm(args: Args, accelerator, tokenizer, model, ref_model, dataloader):
    ...
    with torch.no_grad():
        ...
        for data in tqdm(dataloader):
            ...
            reward_preferred = args.beta * (chosen_logps - 1 * ref_chosen_logps).detach()
            reward_rejected = args.beta * (rejected_logps - 1 * ref_rejected_logps).detach()
...
def evaluate_policy(args: Args, reward_model, model, ref_model, tokenizer, dataloader, generation_config, sampling=True):
...
if __name__ == "__main__":
    ...
    dataset = dataset.with_format(
        ...
        columns=[
            ...
            # "batch",
            # "split",
            f"iter_{args.iter - 1}_query_response_0",
            f"iter_{args.iter - 1}_mask_0",
            # f"iter_{args.iter - 1}_reward_0",
            # f"iter_{args.iter - 1}_logprob_0",
            # f"iter_{args.iter - 1}_query_response_1",
            # f"iter_{args.iter - 1}_mask_1",
            # f"iter_{args.iter - 1}_reward_1",
            # f"iter_{args.iter - 1}_logprob_1",
            # "query_token_len"
        ],
    )
    ...
    for split in ["validation", "validation_cnndm"]:
        validation_dataset = load_dataset("cleanrl/summarize_from_feedback_oai_preprocessing_1705009345", split=split).flatten()
    ...
    local_reward_model = local_reward_model.to(device)
    ...
    # print(evaluate_policy(args, reward_model, accelerator.unwrap_model(model), ref_model, tokenizer, sft_validation_dataloader, validation_generation_config, sampling=True))
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for data in tqdm(dataloader):
            ...
            # trunc_idx = 512 - data["query_token_len"]
            # for i in range(4):
            #     data[f"iter_{args.iter - 1}_query_response_0"][i] = torch.cat([data[f"iter_{args.iter - 1}_query_response_0"][i][trunc_idx[i]:],
            #                                                                    data[f"iter_{args.iter - 1}_query_response_0"][i][:trunc_idx[i]]])
            #     eos = first_true_indices(data[f"iter_{args.iter - 1}_query_response_0"][i] == tokenizer.eos_token_id) + 1
            #     data[f"iter_{args.iter - 1}_query_response_0"][i][eos:] = tokenizer.pad_token_id
            #     data[f"iter_{args.iter - 1}_query_response_1"][i] = torch.cat([data[f"iter_{args.iter - 1}_query_response_1"][i][trunc_idx[i]:],
            #                                                                    data[f"iter_{args.iter - 1}_query_response_1"][i][:trunc_idx[i]]])
            #     eos = first_true_indices(data[f"iter_{args.iter - 1}_query_response_1"][i] == tokenizer.eos_token_id) + 1
            #     data[f"iter_{args.iter - 1}_query_response_1"][i][eos:] = tokenizer.pad_token_id
            # c1 = data[f"iter_{args.iter - 1}_query_response_0"]
            # c2 = data[f"iter_{args.iter - 1}_query_response_1"]


            # rewards_0 = get_reward(reward_model, c1, tokenizer, 512)
            # rewards_1 = get_reward(reward_model, c2, tokenizer, 512)

            # mb_best = (rewards_0[1] < rewards_1[1]).long()

            # query_responses = torch.cat(
            #     [c1.unsqueeze(1), c2.unsqueeze(1)], dim=1
            # ).flatten(0, 1)
            ...
            # for i in range(4):
            #     c1[i][:data["query_token_len"][i]] = tokenizer.pad_token_id
            #     c2[i][:data["query_token_len"][i]] = tokenizer.pad_token_id
            #     c1[i][-trunc_idx[i]:] = tokenizer.pad_token_id
            #     c2[i][-trunc_idx[i]:] = tokenizer.pad_token_id

            # labels = torch.cat(
            #     [c1.unsqueeze(1), c2.unsqueeze(1)],
            #     dim=1,
            # ).flatten(0, 1)
            ...
            # query_responses = torch.cat(
            #     [data[f"iter_{args.iter - 1}_query_response_0"].unsqueeze(1), data["query_reference_response_token"].unsqueeze(1)], dim=1
            # ).flatten(0, 1)

            # c1 = data[f"iter_{args.iter - 1}_query_response_0"].clone()
            # c2 = data[f"iter_{args.iter - 1}_query_response_1"].clone()
            # c1[:, :512] = tokenizer.pad_token_id
            # c2[:, :512] = tokenizer.pad_token_id
            ...
            # labels =  torch.cat(
            #     [data[f"iter_{args.iter - 1}_mask_0"].unsqueeze(1), expert_label.unsqueeze(1)], dim=1
            # ).flatten(0, 1)

            # rewards_0 = get_local_reward(reward_model, data[f"iter_{args.iter - 1}_query_response_0"], tokenizer, 512)
            # rewards_1 = get_local_reward(reward_model, data[f"iter_{args.iter - 1}_query_response_1"], tokenizer, 512)

            # rewards_0 = get_local_reward(local_reward_model, data[f"iter_{args.iter - 1}_query_response_0"], data[f"iter_{args.iter - 1}_mask_0"], tokenizer)
            # rewards_1 = get_local_reward(local_reward_model, data[f"iter_{args.iter - 1}_query_response_1"], data[f"iter_{args.iter - 1}_mask_1"], tokenizer)

            # print(rewards_0, rewards_1)
            
            # data_rewards_0 = data[f"iter_{args.iter - 1}_reward_0"]
            # data_rewards_1 = data[f"iter_{args.iter - 1}_reward_1"]
    
            # rewards_0 = get_reward(reward_model, data["query_response0_token"], tokenizer, 512)
            # rewards_1 = get_reward(reward_model, data["query_response1_token"], tokenizer, 512)

            # rewards_0 = torch.where(data_rewards_0 <= -1e5, data_rewards_0, rewards_0)
            # rewards_1 = torch.where(data_rewards_1 <= -1e5, data_rewards_1, rewards_1)

            # rewards_0 = data_rewards_0
            # rewards_1 = data_rewards_1

            # print(data["query_reference_response_token"].shape, expert_label.shape, data[f"iter_{args.iter - 1}_query_response_0"].shape, data[f"iter_{args.iter - 1}_mask_0"].shape)

            # mb_best = torch.ones(data["query_reference_response_token"].shape[0], device=device) # always pick the expert response

            # print(mb_best.shape)

            expert_query_responses = torch.cat([data["query_token"], data["reference_response_token"]], dim=1)
            expert_label = expert_query_responses.clone()
            expert_label[:, :512] = tokenizer.pad_token_id

            query_responses = torch.cat([data[f"iter_{args.iter - 1}_query_response_0"].unsqueeze(1), expert_query_responses.unsqueeze(1)], dim=1).flatten(0, 1)
            labels = torch.cat([data[f"iter_{args.iter - 1}_mask_0"].unsqueeze(1), expert_label.unsqueeze(1)], dim=1).flatten(0, 1)

            mb_best = torch.ones(data["query_token"].shape[0], device=device).long() # always pick the expert response


            with torch.no_grad():
                ref_chosen_logps, ref_rejected_logps = forward(ref_model, query_responses, labels, mb_best, tokenizer)
                # ref_chosen_logps = get_local_reward(ref_model, data["query_reference_response_token"], expert_label, tokenizer)
                # ref_chosen_logps = get_local_reward(ref_model, expert_query_responses, expert_label, tokenizer)
                # ref_rejected_logps = get_local_reward(ref_model, data[f"iter_{args.iter - 1}_query_response_0"], data[f"iter_{args.iter - 1}_mask_0"], tokenizer)
            with accelerator.accumulate(model):
                chosen_logps, rejected_logps = forward(model, query_responses, labels, mb_best, tokenizer)

                # chosen_logps = get_local_reward(model, data["query_reference_response_token"], expert_label, tokenizer)
                # chosen_logps = get_local_reward(model, expert_query_responses, expert_label, tokenizer)
                # rejected_logps = get_local_reward(model, data[f"iter_{args.iter - 1}_query_response_0"], data[f"iter_{args.iter - 1}_mask_0"], tokenizer)
                # chosen_logps = rejected_logps

                # kl_chosen = chosen_logps - ref_chosen_logps
                # kl_rejected = rejected_logps - ref_rejected_logps
                # reward_chosen = rewards_1[1] * mb_best + rewards_0[1] * (1 - mb_best)
                # reward_rejected = rewards_0[1] * mb_best + rewards_1[1] * (1 - mb_best)
                # total_reward_chosen = reward_chosen - args.beta * kl_chosen
                # total_reward_rejected = reward_rejected - args.beta * kl_rejected
                # true_best = (total_reward_chosen > total_reward_rejected).long()

                pi_logratios = chosen_logps - rejected_logps
                # pi_logratios = pi_logratios * (mb_best == true_best).float() - pi_logratios * (mb_best != true_best).float()

                ref_logratios = ref_chosen_logps - ref_rejected_logps
                # ref_logratios = ref_logratios * (mb_best == true_best).float() - ref_logratios * (mb_best != true_best).float()

                logits = pi_logratios - 1 * ref_logratios  # also known as h_{\pi_\theta}^{y_w,y_l}
                if args.ipo:
                ...
                # loss = loss * torch.logical_not(neither_eos).float()
                loss = loss.mean()
    ...
    if args.run_eval:
        _, evaluate_df = evaluate_policy(args, reward_model, accelerator.unwrap_model(model), ref_model, tokenizer, sft_validation_dataloader, validation_generation_config, sampling=False)
        if accelerator.is_main_process:
