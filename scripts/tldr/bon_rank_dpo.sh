#!/bin/bash
#SBATCH --job-name=eval_bon_dpo_1.4
#SBATCH --output=eval_bon_dpo_1.4.out
...
echo "Gen: $gen"
echo "DPO Reward Model: $reward_model"
...
--main_process_port '${r}' \
--num_processes 1 \
src/tldr/bon_rank_vllm.py \
--eval_df_path /data/user_data/gswamy/bon/'${gen}'/table.csv \
--local_reward_model /data/user_data/gswamy/models/models/'${reward_model}' \
--use_ref True \
